...
class MPagerSnapHelper : SnapHelper() {
    ...
    override fun findTargetSnapPosition(layoutManager: RecyclerView.LayoutManager, velocityX: Int, velocityY: Int): Int {
        var position = 0
        if (layoutManager is MPagerLayoutManager) {
    ...
    override fun findSnapView(layoutManager: RecyclerView.LayoutManager): View? {
        ...
        layoutManager.getChildAt(minChildIndex)?.apply {
            ...
            page = (layoutManager as MPagerLayoutManager).frames[position].page
        }
        // 找到这个page的第一个view并返回
        for ((i, frame) in (layoutManager as MPagerLayoutManager).frames.withIndex()) {
