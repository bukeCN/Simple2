...
public class Fragment implements ComponentCallbacks, OnCreateContextMenuListener, LifecycleOwner,
    ...
    static final int INITIALIZING = -1;          // Not yet attached. 初始化状态
    static final int ATTACHED = 0;               // Attached to the host. 被添加到了宿主
    static final int CREATED = 1;                // Created. 创建
    static final int VIEW_CREATED = 2;           // View Created. 视图创建
    static final int AWAITING_EXIT_EFFECTS = 3;  // Downward state, awaiting exit effects 下行状态，等待退出效果执行完成
    static final int ACTIVITY_CREATED = 4;       // Fully created, not started. 完全创建好了，但是还没有启动
    static final int STARTED = 5;                // Created and started, not resumed. 创建好了，开是启动了，还未显示
    static final int AWAITING_ENTER_EFFECTS = 6; // Upward state, awaiting enter effects 上行状态，等待显示效果执行完成
    static final int RESUMED = 7;                // Created started and resumed. 显示状态，可与用户交互
