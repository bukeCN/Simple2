...
public abstract class FragmentManager implements FragmentResultOwner {
    ...
    boolean execPendingActions(boolean allowStateLoss) {
        ...
        // 重点 1 将待处理的事务添加到 mTmpRecords 中，对应记录是否 pop 弹栈
    ...
    private void removeRedundantOperationsAndExecute(@NonNull ArrayList<BackStackRecord> records,
        ...
        for (int recordNum = 0; recordNum < numRecords; recordNum++) {
            final boolean canReorder = records.get(recordNum).mReorderingAllowed; // 是否允许重排序，看 transation 的注释，默认不允许
            if (!canReorder) {
    ...
    private void executeOpsTogether(@NonNull ArrayList<BackStackRecord> records,
        ...
        Fragment oldPrimaryNav = getPrimaryNavigationFragment(); // 貌似是 jitPack 导航组件的
        for (int recordNum = startIndex; recordNum < endIndex; recordNum++) {
            final BackStackRecord record = records.get(recordNum);
            final boolean isPop = isRecordPop.get(recordNum);
            if (!isPop) {
                // 没看懂，这里就对 List 进行了增删操作
        ...
        // 重点，执行 Fragment 状态管理，建立 FragmentStateManager 进行状态管理
    ...
    private boolean generateOpsForPendingActions(@NonNull ArrayList<BackStackRecord> records,
        ...
        synchronized (mPendingActions) {
            ...
            for (int i = 0; i < numActions; i++) {
                // 重点，generateOps 中会将当前事务添加到回退栈，如果设置了添加
